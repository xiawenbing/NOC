...
class test_transmitDatapotMUL(cfglen:Int,keylen:Int,datalen:Int,resultlen:Int) extends  Module {
    ...
    */ 
    ...
    val potmul_timgReg = RegInit(160000.U(64.W))
...
class Top_potCCnetworkadaptor(inPorts:Int) extends  Module {
    val io = IO(new NetworkInterfaceArbiterBundle_Top(inPorts))
    val trans_pot = Module(new potmultest(1,0,2,2))
    ...
    test_module.io.output<>trans_pot.io.NetworkInterfaceDataIN
    top_queue.io.enq<>trans_pot.io.NetworkInterfaceDataOUT  
    io.output<>top_queue.io.deq 

    io.runCount:=trans_pot.io.runCount
}

//cfg:: 1，0，2,2
class potmultest(cfglen:Int,keylen:Int,datalen:Int,resultlen:Int) extends Module{
    ...
    val transmitdataMUL = Module(new test_transmitDatapotMUL(cfglen:Int,keylen:Int,datalen:Int,resultlen:Int))
    ...
    queue_in.io.deq<>transmitdataMUL.io.NetworkInterfaceDataIN

    queue_out.io.enq<>transmitdataMUL.io.NetworkInterfaceDataOUT
    io.NetworkInterfaceDataOUT<>queue_out.io.deq 

    transmitdataMUL.io.NetworkInterfaceuserOUT<>CC.io.NetworkInterfaceuserIn
    CC.io.NetworkInterfaceuserOUT<>transmitdataMUL.io.NetworkInterfaceuserIn 
    CC.io.runned:=transmitdataMUL.io.runned
    io.runCount:=transmitdataMUL.io.runCount
