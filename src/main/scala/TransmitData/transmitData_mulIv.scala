...
class test_transmitDatamuliv(cfglen:Int,keylen:Int,datalen:Int,resultlen:Int) extends  Module {
    ...
    */ 
    ...
    val potiv_timgReg = RegInit(460.U(64.W))
    ...
    switch(stateReg){
        ...
        is(waitresult){
            potiv_timgReg:=potiv_timgReg-1.U
            when(potiv_timgReg === 0.U){
        ...
        is(done){
                ...
                potiv_timgReg:=460.U //模逆 460时钟周期
...
class Top_mulivnetworkadaptor(inPorts:Int) extends  Module {
    val io = IO(new NetworkInterfaceArbiterBundle_Top(inPorts))
    val trans_muliv= Module(new mulivtest(1,0,2,2))
    ...
    test_module.io.output<>trans_muliv.io.NetworkInterfaceDataIN
    top_queue.io.enq<>trans_muliv.io.NetworkInterfaceDataOUT  
    io.output<>top_queue.io.deq 

    io.runCount:=trans_muliv.io.runCount
}

//cfg:: 1，0，2,2
class mulivtest(cfglen:Int,keylen:Int,datalen:Int,resultlen:Int) extends Module{
    ...
    val transmitdatamuliv = Module(new test_transmitDatamuliv(cfglen:Int,keylen:Int,datalen:Int,resultlen:Int))
    ...
    queue_in.io.deq<>transmitdatamuliv.io.NetworkInterfaceDataIN

    queue_out.io.enq<>transmitdatamuliv.io.NetworkInterfaceDataOUT
    io.NetworkInterfaceDataOUT<>queue_out.io.deq 

    transmitdatamuliv.io.NetworkInterfaceuserOUT<>CC.io.NetworkInterfaceuserIn
    CC.io.NetworkInterfaceuserOUT<>transmitdatamuliv.io.NetworkInterfaceuserIn 
    CC.io.runned:=transmitdatamuliv.io.runned
    io.runCount:=transmitdatamuliv.io.runCount
